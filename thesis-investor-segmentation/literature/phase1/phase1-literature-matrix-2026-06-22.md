...
> **整併狀態：已併入 `../literature-matrix.md`。** 本檔保留為 Research Assistant 原始補強紀錄與查證來源備份；後續正式閱讀與引用請優先使用 `../literature-matrix.md` 的整合版文獻矩陣。

任務範圍：針對「應用投資人行為分群與個人化推薦模型於數位券商基金投資服務之研究」蒐集近 10 年相關學術文獻、產業/官方報告與方法論資料。

重要限制與查證狀態：
- 本檔為 Phase 1 補強矩陣，應搭配既有主矩陣 `../literature-matrix.md` 使用；後續需併回 A–G 分類並去重，不宜維持獨立 1–22 編號作為正式引用系統。
...
- 已知可能重複項目包含：GraphDCF / Chou et al.、Dynamic Utility Learning / Wei & Liu、RFM + K-means、Robo-advisor / investor profiling、IOSCO AI 報告。
