...
- [ ] 待確認｜明確列出本研究不做事項清單（方向已定：著重根據用戶行為的後續研究；仍需列出具體排除項，如是否排除完整 robo-advisor、報酬預測、投資績效保證、全新演算法開發）
- [x] 2026-09-15 已確認｜研究期間：最近 12–18 個月、有交易資料的用戶
- [x] 2026-09-15 已確認｜推薦模型方向：保留完整推薦模型（不降級為概念驗證），組合為熱門推薦 → 內容導向推薦 → 協同過濾／ALS 矩陣分解 → 分群輔助 CF
- [ ] 待確認｜收斂 2–3 個可用資料回答的研究問題（草稿見 `thesis-chapter-progress.md` 1.2）
- [ ] 待確認｜資料授權：尚未跟公司口頭確認，計畫先產出研究架構，再以「執行行銷專案」方向跟老闆討論

**Phase 0 里程碑 🟡**：題目方向、研究範圍、對象、期間、推薦模型組合已於 2026-09-15 確認；不做事項清單與資料授權仍待補，補齊後才可進入 Proposal 定稿。
