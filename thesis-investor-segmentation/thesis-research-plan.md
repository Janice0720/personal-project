...
整合網站使用者行為資料（可能結合 GA4、BigQuery、內部資料庫等來源）與基金交易資料，建立投資人行為分群模型，並依序建立熱門推薦、內容導向推薦、協同過濾（ALS 隱性回饋矩陣分解）、分群輔助 CF 四種推薦模型，評估其相較熱門推薦 baseline 是否具有增量效果。基金可交易型錄約 4,000+ 檔、活躍交易用戶數量不少，型錄規模足以支撐協同過濾 / 矩陣分解類方法；本研究仍不以提出全新演算法為目標，而是把既有可解釋方法（分群 + CF）組合成適合數位券商基金場景的推薦流程。（2026-09-15 更新：由「概念驗證」調整為完整推薦模型組合）
...
推薦（依基金型錄 4,000+ 檔、活躍交易用戶不少的規模重新評估）：
熱門推薦(Baseline) ──→ 內容導向推薦 ──→ 協同過濾 / ALS 矩陣分解 ──→ 分群輔助 CF（核心貢獻）
   （比較基準）         （基金屬性相似度，補冷啟動）  （implicit 套件，隱性回饋）    （分群縮小候選集或作為 CF 輸入特徵）
```

型錄達 4,000+ 檔後，Item-Based / User-Based CF 與矩陣分解才真正有意義（型錄小時退化成簡單相似規則）；建議以 ALS（`implicit` 套件）取代 `scikit-surprise` 的顯性評分 SVD，因交易資料屬於「有買/沒買」的隱性正向回饋。分群輔助 CF 是把研究問題一（分群）與研究問題二（推薦）串起來的差異化貢獻，不是另外的獨立方法。
