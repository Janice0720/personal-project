...
整合使用者行為資料（GA4）與基金交易資料，建立投資人行為分群模型，並依序建立熱門推薦、內容導向推薦、協同過濾（ALS 隱性回饋矩陣分解）、分群輔助 CF 四種推薦模型，評估其相較熱門推薦 baseline 是否具有增量效果。基金可交易型錄約 4,000+ 檔、活躍交易用戶數量不少，型錄規模足以支撐協同過濾 / 矩陣分解類方法；本研究仍不以提出全新演算法為目標，而是把既有可解釋方法（分群 + CF）組合成適合數位券商基金場景的推薦流程。（2026-09-15 更新：由「概念驗證」調整為完整推薦模型組合）
